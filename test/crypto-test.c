...
static the_str_t convert_byte_array_to_str (const char *buf, unsigned int len) {
  const unsigned char hex[] = "0123456789ABCDEF";
  wchar_t *wide_buf = the_safe_alloc((len * 2 + 1) * sizeof(wchar_t));
  the_str_t result;
...
static the_str_t crypto_sha256 (const char *msg) {
  char *buf;
  unsigned int len;
  the_str_t result;
  ...
  buf = the_safe_alloc(EVP_MD_get_size(sha256));
...
static void test_crypto (void) {
  ...
  assert(((void) "SHA256 works", the_str_eq(s, s_cmp)));
